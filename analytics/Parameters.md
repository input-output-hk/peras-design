...
| Active slot coefficient | $\alpha$ | 1/slots | The probability that a party will be the slot leader for a particular slot. | $0 < \alpha \leq 1$ | Blocks must be produced. |
...
| Chain-quality time | $T_\text{CQ}$ | slots | Ensure the presence of at least one honest block on the chain. | $T_\text{CQ} ≟ \mathcal{O} \left( \left( \alpha\log (1 - \alpha) \right)^{-1} \right)$ | A least one honest block must be produced. |
