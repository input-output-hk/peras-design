...
data EnvState = MkEnvState{lastBlock :: Block,
                           lastBlockTime :: Slot, sutParty :: Party, time :: Slot}
                  deriving (Eq, Ord, Show, Generic)
...
tickSlot s
  = MkEnvState (lastBlock s) (lastBlockTime s) (sutParty s)
      (1 + time s)

data WhenTick = GenesisTick
              | TickAfterEnvSend
              | SutSendAndTick
              | NoTick
...
whenTick s
  = if envSentBlock s then TickAfterEnvSend else
      if time s == 0 then GenesisTick else
        if whoseSlot s == sutParty s then SutSendAndTick else NoTick
