...
peersGraph networkState =
  let
    nodeStates = networkState ^. exitStates
    ...
    mkNode name nodeState =
      G.NodeStatement (nodeIds M.! name)
        ...
        , G.AttributeSetValue (G.NameId "label") . G.XmlId . G.XmlText
           $ "<b>" <> show name <> "</b>"
           <> "|stake=" <> show (nodeState ^. stake)
           <> "|vrfOutput=" <> take 6 (show $ nodeState ^. vrfOutput)
           <> "|slotLeader=" <> show (nodeState ^. slotLeader)
           <> "|committeeMember=" <> show (nodeState ^. committeeMember)
        ]
...
chainGraph networkState =
  let
    chains = (^. preferredChain) <$> M.elems (networkState ^. exitStates)
    ...
    mkNode Block{..} =
      G.NodeStatement (nodeId signature)
        [
          G.AttributeSetValue (G.NameId "shape") (G.StringId "record")
        , G.AttributeSetValue (G.NameId "label") . G.XmlId . G.XmlText
            $ "<b>" <> show signature <> "</b>"
            <> "|slot=" <> show slotNumber
            <> "|creator=" <> show creatorId
            <> "|votes=" <> show includedVotes
        ]
    blocks Genesis = []
