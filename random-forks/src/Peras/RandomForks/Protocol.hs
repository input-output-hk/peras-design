...
mkProtocol Parameters{..} =
  let expectedStake = fromIntegral $ peerCount * maximumCurrency `div` 2
...
isSlotLeader gen Protocol{pSlotLottery} currency =
  ...
  let p = 1 - (1 - pSlotLottery) ^ currency
   in (<= p) <$> uniformRM (0, 1) gen
...
isCommitteeMember gen Protocol{pCommitteeLottery} currency =
  let p = 1 - (1 - pCommitteeLottery) ^ currency
   in (<= p) <$> uniformRM (0, 1) gen
