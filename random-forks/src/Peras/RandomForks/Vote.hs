...
allVotes chains dangling =
  let
    attached = foldMap (S.unions . fmap votes . blocks) chains
  in
    S.union attached dangling
...
refreshVotes votes' chain =
  ...
  in
    (votes' `S.difference` foldMap votes (blocks chain'), chain')
...
recordVote votingRound voter (minSlot, maxSlot) chain =
  let
    recordVote' Genesis = Genesis
    recordVote' Chain{..} =
      if minSlot <= slot block && slot block <= maxSlot
        then let
               votedBlock = blockId block
             in
               Chain (block {votes = Vote{..} `S.insert` votes block}) prev
