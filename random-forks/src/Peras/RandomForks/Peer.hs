...
randomPeers gen Parameters{..} protocol =
  do
    let peerNames = PeerName . ("P" <>) . show <$> [1 .. peerCount]
    ...
    let upstreams = M.fromListWith (<>) . concatMap (\(name, names) -> (,S.singleton name) <$> S.toList names) $ M.toList downstreams
        randomPeer name =
          do
...
nextSlot gen protocol@Protocol{roundDuration, votingWindow} slot name state@PeerState{..} =
  do
    ...
    let chainWeight' = chainWeight $ Protocol.votingBoost protocol
    ...
    let (danglingVotes', preferredChain'') = refreshVotes votes' preferredChain'
        newMessages =
          if slotLeader'
    ...
    let
      ...
      newState =
        state
          ...
          , preferredChain =
              if committeeMember' && slot `mod` roundDuration == 0 -- FIXME: Not faithful to the Peras voting rules.
