...
# Testing with Dynamic QuickCheck
...
- Separation between "idealized" votes, certificates, blocks, and chains of the specification from "realized" ones generated by `agda2hs` and used in the simulations.
    - The idealized version ignores some details like signatures and proofs.
    - It is possible to remove this separation between ideal and real behavior is fully deterministic (including the bytes of signatures and proofs).
    - However, this prototype demonstrates the feasibility of having a slightly more abstract model of a node for use in `Test.QuickCheck.StateModel`.
...
The model is implemented by the following Haskell types and instances.
...
> [!NOTE] Are bullets okay, or should we organize this into paragraphs?
