...
extend ys l = assert (l >= length ys && not (null ys))
...
liftBinaryOp :: (Double -> Double -> Double)
             -> NCDF -> NCDF -> NCDF
...
convN (Tab dx1 fv) (Tab dx2 gv) dx
  ...
  $ Tab dx $ integrate (conv (grad fv dx) (grad gv dx) dx) dx
...
instance A.CDF NCDF where
  ...
  tabulate (Fun m f) dx = let fv = tabulate f m dx
                          in zip (xvalues (length fv) dx) fv
  tabulate (Tab dx ys) dx' = assert (dx == dx')
