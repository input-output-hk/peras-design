...
toBoxPlusList (Plus w1 w2 a b) m =
  let s = fromIntegral $ w1 + w2
...
boxToDot b =
 ...
 where
   ...
   where
    doLabel (i, n, leaf) =
      printf
        ...
        n
        ( if leaf
            then ",shape=circle,style=filled"
            else ",style=rounded"
        )
  ...
  edges (Node t@(BTag{nodeId = i, cluster = Just (ci, cn)}) children) =
    if null children
      then subgraph ci (escape cn) (printf "  %d;\n" i)
      else subgraph ci (escape cn) (edges (Node (t{cluster = Nothing}) children))
