...
data DeltaQ p (d :: Type -> Type) n where
  ...
  Convolve ::
    ...
    DeltaQ p d n ->
    DeltaQ p d n
  NToFinish :: Int -> [DeltaQ p d n] -> DeltaQ p d n
...
instance
  ...
  fromRational r
    | r < 0 || r > 1 =
        error $
          "DeltaQ is a probability, <0 or >1 undefined - given '"
            ++ show r
            ++ "'"
    | r == 0 = Bottom
    | r == 1 = Unit
...
instance
  ...
  toRational x = case tangibleMass x of
    ...
    y ->
      error $
        "un-normalised tangibleMass representation '"
          ++ simpleShow y
          ++ "' - hence toRational is not defined"
