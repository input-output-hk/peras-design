...
class ImproperRandomVar m where
  tangibleMass ::
    m ->
    -- | <= unit probability. 1 - epsilon
    m
  tangibleRandomness ::
    m ->
    -- | a proper random variable
...
instance
  (Fractional p, Real p, DelayModel d n) =>
  ImproperRandomVar (DeltaQ p d n)
  where
  ...
  tangibleMass (ProbChoice p a b) =
    fromRational $
      (toRational p) * (toRational $ tangibleMass a)
...
mmmvChoice ::
  (Fractional p, Real p, Ord a, Fractional a) =>
  p ->
  MinMaxMeanVar a ->
  MinMaxMeanVar a ->
  MinMaxMeanVar a
mmmvChoice p a b =
  MMMV
    { mmmvMin = (mmmvMin a) `min` (mmmvMin b)
