...
data Vote = MkVote{votingRound :: RoundNumber,
                   creatorId :: PartyId, proofM :: MembershipProof,
                   blockHash :: Hash Block, signature :: Signature}
              deriving (Generic)
    ...
    x == y
      = votingRound x == votingRound y &&
          creatorId x == creatorId y &&
            proofM x == proofM y &&
              blockHash x == blockHash y && signature x == signature y
