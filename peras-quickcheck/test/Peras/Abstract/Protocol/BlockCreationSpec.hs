...
import Peras.Abstract.Protocol.BlockCreation
...
spec = do
  let params = defaultParams
      ...
      slotLeader = MkParty (fromIntegral roundNumber) (arbitrary `generateWith` 42)
      _nonSlotLeader = MkParty (fromIntegral roundNumber + 1) (arbitrary `generateWith` 42)
      steadyState =
        initialPerasState
          { chainPref = someChain
