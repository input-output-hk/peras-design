...
The module [`Peras.ChainWeight`](peras-quickcheck/src/Peras/ChainWeight.hs] demonstrates how Agda code that uses the standard library (not `agda2hs`) can be called from a Haskell function via stable, unmangled names (types and functions) generated by `agda` and `agda2hs`. The basic recipe is as follows:
